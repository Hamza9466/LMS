    ...
    @if($isVideo)
        ...
        <div class="mt-3 d-flex align-items-center gap-3" @if($showQuizFlag ?? false) style="display:none" @endif>
            ...
            <button id="nextBtn" class="btn btn-primary" {{ ($watchedOK ?? false) ? '' : 'disabled' }}>
                {{ ($watchedOK ?? false) ? 'Start MCQs' : 'Next' }}
            </button>
            @if(($watchedOK ?? false) && !empty($quizId))
                <div class="text-success small">
                    <i class="fas fa-check-circle"></i> MCQs will start automatically when video reaches 90%
                </div>
            @endif
        </div>
...
(function(){
  ...
  function bump(pct, force=false){
    ...
    if (pct > maxPercent || force) {
      ...
      if (maxPercent >= 90 && nextBtn && nextBtn.hasAttribute('disabled')) {
        ...
        // Auto-show MCQs when 90%+ is reached
        autoShowMCQs();
      }
  ...
  // Auto-show MCQs function
  function autoShowMCQs() {
    console.log('Auto-showing MCQs...');
    // Show a notification that MCQs are starting
    showMCQNotification();
    // Redirect to show quiz directly
    const url = new URL(window.location.href);
    url.searchParams.set('showquiz','1');
    window.location.href = url.toString();
  }

  // Show notification that MCQs are starting
  function showMCQNotification() {
    // Create notification element
    const notification = document.createElement('div');
    notification.className = 'alert alert-info alert-dismissible fade show position-fixed';
    notification.style.cssText = 'top: 20px; right: 20px; z-index: 9999; min-width: 300px;';
    notification.innerHTML = `
      <strong>MCQs Ready!</strong> Video completed. Starting quiz in a moment...
    ...
    function startTimer(){
      ...
      timer = setInterval(async () => {
        if (!sending) {
          ...
          // Check if we should auto-show MCQs for HTML5 videos
          if (pct >= 90 && maxPercent < 90) {
            autoShowMCQs();
          }
        }
      }, 2000);
  ...
  async function updateYTProgress(force=false){
    ...
    // Check if we should auto-show MCQs for YouTube videos
    if (pct >= 90 && maxPercent < 90) {
      autoShowMCQs();
    }
  }
