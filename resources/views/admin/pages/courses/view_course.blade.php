...
@php
    // Helpers for display
    $finalPrice = $course->final_price;               // from model accessor
    $percentOff = $course->percent_off;               // from model accessor
    $isFree     = $course->is_free;

    $durationM  = $course->total_duration_minutes;
    $duration   = $durationM ? (floor($durationM/60).'h '.($durationM%60).'m') : '—';

    $teacherName = $course->teacher?->teacherDetail
        ? trim(($course->teacher->teacherDetail->first_name ?? '').' '.($course->teacher->teacherDetail->last_name ?? ''))
        : ($course->teacher->name ?? '—');
...
<div class="container py-4">
 ...
 <div class="card-body">
            <p><strong>Category:</strong> 
                {{ $course->category ? $course->category->name : 'No category assigned' }}
            </p>
 </div>
    ...
    @php
        ...
        if ($host && str_contains($host, 'youtu.be')) {
            ...
            $id = ltrim(parse_url($raw, PHP_URL_PATH) ?? '', '/');
            $embed = $id ? "https://www.youtube.com/embed/{$id}" : null;

        } elseif ($host && str_contains($host, 'youtube.com')) {
            // https://www.youtube.com/watch?v=VIDEOID or /embed/VIDEOID or /shorts/VIDEOID
            parse_str(parse_url($raw, PHP_URL_QUERY) ?? '', $q);
            $id = $q['v'] ?? null;
            if (!$id) {
                $path = parse_url($raw, PHP_URL_PATH) ?? '';
                if (preg_match('#/(embed|v|shorts)/([^/?]+)#', $path, $m)) {
                    $id = $m[2];
                }
            }
            $embed = $id ? "https://www.youtube.com/embed/{$id}" : null;
            ...
            // https://vimeo.com/VIDEOID or https://player.vimeo.com/video/VIDEOID
            $path = trim(parse_url($raw, PHP_URL_PATH) ?? '', '/');
            if (preg_match('#^(\d+)$#', $path, $m)) {
                $embed = "https://player.vimeo.com/video/{$m[1]}";
            } elseif (preg_match('#video/(\d+)#', $path, $m)) {
                $embed = "https://player.vimeo.com/video/{$m[1]}";
            }
        }
    @endphp
    ...
    <div class="mb-4">
        <div class="text-muted small mb-2">Promo Video</div>
        ...
        @else
            {{-- Fallback: show a clickable link if we couldn't parse --}}
